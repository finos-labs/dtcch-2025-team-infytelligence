        ...
        with st.form(key="search_form", clear_on_submit=False):
            ...
            with col1:
                search_id = st.text_input(
                    "Enter Document ID to Extract:", key="search_input"
                )
            with col2:
            ...
            if submit_button and search_id != st.session_state.last_search_id:
                st.session_state.search_results = search_records(
                    st.session_state.df, search_id
                )
    ...
    with results_container:
        if st.session_state.search_results is not None:
            if not st.session_state.search_results.empty:
                ...
                st.warning(
                    f"No records found for Document ID: {st.session_state.last_search_id}"
                )
